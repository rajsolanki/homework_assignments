...
count.pop(6) + count.push(8, 9, 10)
puts "If we delete 5, 6 and 7 from the array, we're left with #{count}"
puts "If we add 5 at the beginning of the array, we're left with #{count.unshift(5)}"
puts "If we add 6 at the end fo the array, we're left with #{count.push(6)}"
puts "Only the elements #{count[6..7]} are > #{count[5].to_s}"
count.pop(9)
puts "If we remove all the elements, then the length of the array is #{count.length}"
